...
static void init_msgq(struct procent *prptr)
{
	struct mesgnode *next, *cur, *mfree;
	uint16 msgcnt;

	/* create semaphores for send and receive */
	prptr->msgq.msgssem = semcreate(MAXPMSG);
	prptr->msgq.msgrsem = semcreate(0);
	prptr->msgq.mqlock  = create_mutex();

	/* initialize message queue and related params */
	prptr->msgq.msgmaxcnt    = msgcnt = MAXPMSG;
	prptr->msgq.mfree        = prptr->msgq.mqueueh = mfree = (struct mesgnode *)getmem(MAXPMSG*sizeof(struct mesgnode)); 
	if (ptfree == (struct ptnode *)SYSERR) {
		panic("ptinit - insufficient memory");
	}

	for (cur=next=mfree; --msgcnt > 0; cur=next ) {	
		cur->mnext = ++next;
	}
