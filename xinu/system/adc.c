 ...
 *
 */
int BBBIO_ADCTSC_channel_ctrl(unsigned int chn_ID, int mode,
                              int open_dly, int sample_dly,
                              int sample_avg, unsigned int *buf,
...
//	unsigned int tmp_channel_en = ADCTSC.channel_en;
	...
	*reg_ctrl |= (CTRL_ENABLE | CTRL_STEP_ID_TAG);
	...
	if(ADCTSC.work_mode & BBBIO_ADC_WORK_MODE_TIMER_INT) {
		/* TBD: need to trigger _ADCTSC_work() periodically
		 * we need to complete the coding. till then we need
		 * to rely on budy-polling only */
...
#ifdef BBBIO_LIB_DBG
    kprintf("BBBIO_ADCTSC_Init >> %x\n", cm_wkup_addr);
#endif

	adctsc_ptr = (unsigned int *)ADCTSC_MMAP_ADDR;
